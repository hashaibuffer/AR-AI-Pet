...
固定七个剧本优先调用 StackChan 固件内置的 `self.scene.play`，由设备本地时间轴执行，减少 MCP 往返和通道等待。只有未编译进固件的自定义剧本才由 Bridge 按 `atMs` 调度；相同时间点的底座、屏幕和灯光动作会并行下发，同一通道内保持顺序。当前不处理打断优先级，保留既有 `robot.command.stop` 路径。语音仍由主 Xiaozhi 链路在剧本前后播放，不与头部动作强行并行。

Adapter 只接收 `nod`、`wave`、`dance`、`scene.play`、`farm_tend`、`stop` 等语义动作，不接收 PWM、电压或电机寄存器参数。

`scene.play` 的参数是 `sceneId`、`durationMs` 和语义 `steps`，首版最大时长为 60 秒，正式七个场景控制在约 4.8—12 秒。固定 `sceneId` 只传递一次 `self.scene.play`；其他场景才由 `SceneStepMapper` 编译成当前固件已有的高层工具调用：
