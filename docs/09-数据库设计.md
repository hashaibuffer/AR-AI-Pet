...
人格和行为规则不复制进数据库。`state_documents` 的 `pet.data` 只保存当前选择的 `personaId`、`personaVersion` 和调度运行时计数；后续事件协议可保存 `behaviorId`、`dialogueKey`、`innerOsKey` 作为当时内容版本的引用。实际文本和规则由仓库 `content/runtime/` 按版本提供。

这样可以在不迁移历史状态的情况下更新台词，同时仍能根据稳定 ID 追溯一次提醒、农场行为或游戏反馈使用的内容。
...
当前实现已提供 `conversation.get` 短期上下文读取，以及 `memory-job.claim/complete/fail/ignore/recover` 数据服务内部消息。Mock Worker 链路已通过；真实 Mem0 仍需外部 LLM 与 Embedding 凭据后单独验收。
