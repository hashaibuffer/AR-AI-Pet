...
| AgentOS | Xiaozhi 主 Agent、项目工具、记忆和生活状态 | Xiaozhi 会话已验证；项目 MCP Hub 尚未挂载 |
...
首版采用 Xiaozhi AI.AGENT 作为唯一主 Agent，通过 AR-AIPet MCP Hub 调用项目工具，不再另建一套 Agent 对话运行时。
...
flowchart LR
  ...
  D --> W["记忆任务 Worker"]
  W --> M["Mem0"]
  S["StackChan"] <--> A["Xiaozhi AI.AGENT"]
  A <--> H["AR-AIPet MCP Hub"]
  H <--> D
...
Unity App 通过 WebSocket 读写项目数据服务；Xiaozhi AI.AGENT 通过 MCP Hub 调用业务和设备工具。PostgreSQL 是业务事实来源，Mem0 由异步 Worker 派生。当前正式数据流为 `MCP Hub → DataServiceClient → WebSocket 数据服务 → PostgreSQL`；MCP Hub 不直接访问数据库。
