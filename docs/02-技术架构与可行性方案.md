...
Beam Pro 只连接项目数据服务，不承担机器人驱动。StackChan 连接 Xiaozhi 会话服务和独立行为 MCP，并通过 BLE 透明串口控制 NanoDrive。NanoDrive 不需要自带 Wi-Fi；移动安全必须在底座本地生效。
...
| stackchan-mcp Gateway | 电脑（:8765/:8767） | 把设备能力包装成 MCP 工具 |
...
| 设备端 McpActionClient | 机器人（C++） | 客户端 | 主动连 :8765 收动作指令，转设备 MCP 执行（不碰语音） |
| 设备固件 MCP | 机器人（StackChan 自带） | 设备能力 | 控屏幕/表情/舵机/底座（**保留不合并**） |

动作链路方向：**服务端 Adapter（发）→ 网络 /ws/device → 设备 McpActionClient（收）→ 设备 MCP 执行**。Adapter 与 McpActionClient 是一对，一发一收，中间隔网络。

### 13.4 基线现状（当前 vs 历史）

| 固件 | commit | 定位 | 能力 |
|---|---|---|---|
| stackchan-mcp（kit 方案） | `21ab6636` | **当前产品固件** | 语音/表情/底座/BLE v0.9/CI 复现（57 Python + 17 主机测试） |
...
| 词 | 含义 |
|---|---|
| 方案 B | ① 底座连接方式（独立 ESP32）→ ② 固件架构（Kimito 为主 + stackchan-mcp Gateway + Robot Bridge）→ ③ 连接协议（scheme-b，设备发 hello+mcp）。当前语境=②的落地产物（stackchan-mcp） |
