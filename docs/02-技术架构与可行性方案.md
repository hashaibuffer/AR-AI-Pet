...
Beam Pro 只连接项目局域网服务（数据与 Agent 会话），不承担机器人驱动。StackChan 连接 Xiaozhi 会话服务和独立行为 MCP，并通过 BLE 透明串口控制 NanoDrive；统一入口下由服务端 Adapter 代发语义动作。NanoDrive 不需要自带 Wi-Fi；移动安全必须在底座本地生效。
...
当前保留两种兼容入口：旧 Compose 拓扑中的本地 Agent Runtime 通过 AR-AIPet MCP Hub 调用项目工具；`unified` profile 中的 Agent 直接调用同一套内置语义工具注册表，外部仍可通过 `/mcp` 接入。两种入口不产生第二套业务状态。Xiaozhi/StackChan 固件继续提供设备语音和实体能力，语音协议网关后续接入。
...
Unity App 通过 WebSocket 读写项目数据服务；旧拓扑中的 Agent Runtime 通过 MCP Hub 调用业务和设备工具，统一拓扑则直接调用同名内置工具。PostgreSQL 是业务事实来源，Mem0 由 Worker 异步派生；Memory Worker 只能通过数据服务 WebSocket 读写任务和 `memory_refs`。旧拓扑的文字闭环为 `用户文字 → Agent Runtime → MCP Hub → DataServiceClient → WebSocket 数据服务 → PostgreSQL → Agent 回复`，统一拓扑将中间 MCP Hub 替换为进程内工具注册表，但业务数据流不变。长期记忆检索为 `Agent Runtime → Memory Service → Mem0/Qdrant`。Xiaozhi/StackChan 语音协议网关后续接入。
...
为减少本地部署时的进程和连接数量，当前实现增加一个可选的统一入口 `ar-aipet-server`。它不是新的业务层，而是把现有 Agent、数据服务、Memory Service、MCP 兼容入口和设备会话放进同一进程；PostgreSQL、Mem0 和确定性游戏规则的职责不变。

```mermaid
flowchart LR
  P["Beam Pro / Unity"] -->|"WebSocket /ws/app"| S["AR-AIPet Server"]
  R["StackChan / 设备端"] -->|"WebSocket /ws/device"| S
  S --> A["Agent + 内置语义工具"]
  S --> D["数据服务"]
  D --> DB["PostgreSQL"]
  S --> M["Memory Service"]
  M --> Q["Mem0 / Qdrant"]
  S --> X["设备 Adapter"]
  E["外部 MCP 客户端"] -->|"/mcp 兼容入口"| S
  X --> R
```

统一服务的内置工具注册表与 FastMCP Hub 使用相同语义工具名；Agent 在统一部署中直接调用内置注册表，外部 `/mcp` 只承担兼容接入，不再形成第二个状态所有者。旧的 `agent-runtime`、`mcp-hub` 和 `robot-bridge` Compose 服务保留为回滚与对照验证路径。

设备端同时兼容项目 `device-v1` 和现有 Scheme B 的 `hello` + `mcp` 消息，因此不要求立即重写固件。动作结果中的 `transportAccepted` 表示网关已发送或收到设备协议消息，`physicalConfirmed` 只有在实体观察完成后才能置为 `true`。
