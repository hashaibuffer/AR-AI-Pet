...
| AgentOS | 本地 Agent Runtime、项目工具、记忆和生活状态 | 文字工具闭环待验证；真实模型和语音网关后续接入 |
...
- Mem0 只保存适合语义检索的长期记忆，不保存游戏胜负和关键数值；当前只预留异步任务接口。
...
| Agent | [AI.AGENT / Xiaozhi 服务](https://xiaozhi.me/)及当前产品固件 | 采用 Xiaozhi AI.AGENT 作为主 Agent 运行时 |
| MCP Hub | [FastMCP](https://github.com/jlowin/fastmcp) | 采用；当前提供项目状态与日程工具 |
| 记忆 | [Mem0](https://github.com/mem0ai/mem0) | 自托管；由 Worker 异步接入，尚未实测 |
...
Unity App 通过 WebSocket 读写项目数据服务；本地 Agent Runtime 通过 MCP Hub 调用业务和设备工具。PostgreSQL 是业务事实来源，Mem0 由异步 Worker 派生。当前文字闭环为 `用户文字 → Agent Runtime → MCP Hub → DataServiceClient → WebSocket 数据服务 → PostgreSQL → Agent 回复`。Xiaozhi/StackChan 语音协议网关后续接入。
